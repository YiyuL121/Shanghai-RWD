...
sample_pars_pop <- function(pop, num){
  ...
  for (i in 1:(length(log10_pop)+length(non_log10_pop))) {
    ...
    if (i <= length(log10_pop)){
      ...
      mean_par <- pop$value[log10_pop[i]]      # μ_log10
      sd_par   <- pop$value[log10_omega[i]]    # ω_log10

      log10_par <- rnorm(num, mean = mean_par, sd = sd_par)
      pars[, i]    <- 10^(log10_par)
      ...
      pars[, i] <- exp(rnorm(n=num, mean=log(mean_par), sd=sd_par))
    ...
    } else if (i == length(log10_pop)+3 & length(log10_pop)+3 == n_prf ) {  # Sample for IC50
...
Intfun_ept_hill <- function(pars, t_treat, antiviral) {
  ...
  delta <- as.numeric(pars[5])
   # To be estimated
  tau <- round(as.numeric(pars[6]),digit=1)    # round the tau, as the same digit of stepsize
  #tau <- 1 #population one
  IC50 <- as.numeric(pars[7])
  ...
  treatment$h <- ifelse(times >= rec, A, 0)  # 5-day treatment
  ...
  derivs <- function(times, y, pars, h_t2) {
    with(as.list(c(pars, y)), {
      ...
      eps <- E_max * (C_P_uM^n) / ((IC50)^n + C_P_uM^n)
...
```
#####################################################
######### Calculate population efficacy #############
#####################################################

```{r}
sample <- pop $value
par_samp <- c(beta = 10^as.numeric(sample[1]), pi = 10^as.numeric(sample[2]), phi =10^as.numeric(sample[3]),
          rho = 10^as.numeric(sample[4]), delta = as.numeric(sample[5]), tau = as.numeric(sample[6]), IC50 = as.numeric(sample[7]))
par_samp
t_1 <- 3
tau_1 <- round((par_samp[6]),digit=1)
# Run the simulation
result <- Intfun_ept_hill(par_samp, t_1,1)
# Compute E_ave
E_ave <- ComputeEave(result, t_1+tau_1)
print(paste("Average efficacy (E_ave):", E_ave))
```
...
for (g in 1:4){
  ...
  t_treat_since_inf <- t_treat +  round(pop$value[6], digit =1)
...
#Original <- read.csv("Paxlovid_match_rpart.csv", row.names = 1)  # Not publicly available
...
Intfun_ind_pkpd <-function(pars){
  ...
  treatment$h <- ifelse(times < rec, 0, 1) * Antiviral #5day treatment
...
ind_fit_plt1<-function(Estimated, Simulated, original){
  ...
  for(i in 1:nrow(Estimated)){
    ...
    Min50  <- apply(P,1,function(x){quantile(x,0.25,na.rm = TRUE)})
    Max50  <- apply(P,1,function(x){quantile(x,0.75,na.rm = TRUE)})
    Max95  <- apply(P,1,function(x){quantile(x,0.975,na.rm = TRUE)})
    Mean50 <- apply(P,1,function(x){quantile(x,0.50,na.rm=T)})
    fit <- cbind(d1,Min95,Min50,Max50,Max95,Mean50,Code)
    fit$Min95[fit$Min95 < -2] <- -2
    fit$Min50[fit$Min50 < -2] <- -2
...
```{r}
selected_sim <- Sim %>%
  dplyr::select(log10beta,log10pi,log10phi,log10rho,delta,tau,IC50,antiviral,T_treat,id) %>%
  rename(ID=id)
selected_sim
```
